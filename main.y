...
int typeDeclaration;// variable global de l'analyseur syntaxique
...
set_nbvariables_globales_to_0: // elle n'a pas de partie droite (une action semantique qui sera executé) 
		{ 
...
set_type_declaration_formels : // utilisation à plusieur endroit à la fois(même règles pour plusieurs contextes)
		{
...
liste_variables : liste_variables ',' T_VARIABLE	{ declarer_variable(getFunctionNum(),$3,typeDeclaration); }// $3 type de la variable
																											  // typeDeclaration c'est une varaible global de l'analyseur syntaxique
...
prototype_fonction: T_FUNCTION T_VARIABLE 
		{
		    // vérifier que le nom de la fonction qui n'existe pas 
			declarer_variable(0,$2,TYPE_FONCTION);
			int index=est_deja_declare(0,$2);
			printf("index=%d\n",index);

			increment_current_function(); // incrémenter le num de la fonction courante
			printf("getFunctionNum()=%d\n",getFunctionNum());
			ajouter_table_fonctions(index,getFunctionNum()); // ajouter le numéro de la fonction à la table des fonctions
			
		  	nodeType *nodeIndex=createNumericNode((float)index); // noeud numeric  (un index)
		  	nodeType *nodeFunctionNum=createNumericNode((float)getFunctionNum()); // noeud numerique pour le numéro de la fonction ()
...
acces_variable : T_VARIABLE 
		{ 
			int varloc=est_deja_declare(getFunctionNum(),$1); // vérifier qu'elle a été déclarer comme variable local
			int varglob=est_deja_declare(0,$1); // cas global
			if (varloc!=-1)
				$$=createIdentifierNode($1,getFunctionNum(),varloc) ; // type Identifiant
			else if (varglob!=-1)
...
int main(void) {
	...
	printf("generateAsm\n");
	FILE * fff=fopen("EDGE","w+");
	generateDigraphEdges(programRoot,fff);
